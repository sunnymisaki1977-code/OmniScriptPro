    ...
    try {
      for (let i = startStep; i <= STEPS.length; i++) {
        ...
        if (isCanvasEnv && localPromptFunctions) {
            ...
            // 🌟 動態選擇模型：Step 1 需要 Google Search 時，切換至 gemini-2.5-flash
            const targetModel = (i === 1 && !safeStep1) ? 'gemini-2.5-flash' : 'gemini-2.5-flash-preview-09-2025';
...
<h2 className="text-2xl md:text-3xl font-black tracking-tight text-[#1E293B]">
                      ...
                      <div className="mt-6 bg-slate-50 border border-slate-200/80 rounded-2xl p-4 md:p-5 text-left space-y-3 shadow-inner">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-medium text-slate-600">
                          ...
                          <div 
                            onClick={() => { setPasscode('fairytales2026'); setAuthError(''); }}
                            className="flex items-center justify-between bg-white px-3 py-2 rounded-xl border border-slate-100 shadow-sm hover:border-emerald-400 hover:shadow transition-all cursor-pointer group/hint"
                            title="點擊帶入 fairytales2026"
                          >
                            <span className="group-hover/hint:text-slate-900 transition-colors truncate mr-2">🧚 文化轉譯・銘印童話</span>
                            <code className="font-mono font-bold text-emerald-600 bg-emerald-50 group-hover/hint:bg-emerald-500 group-hover/hint:text-white px-2 py-0.5 rounded transition-colors shrink-0">fairytales2026</code>
                          </div>
                          <div 
                            onClick={() => { setPasscode('proposal2026'); setAuthError(''); }}
                            className="flex items-center justify-between bg-white px-3 py-2 rounded-xl border border-slate-100 shadow-sm hover:border-emerald-400 hover:shadow transition-all cursor-pointer group/hint"
