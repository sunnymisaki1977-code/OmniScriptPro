...
接著依照對應模板撰寫。
第一類：神佛/歷史人物
請針對主題「${ctx.theme}」進行一份900字精確事實報告。
...
`,  },
  ...
  beauty: [
  {
    ...
    type: "text",
    dependsOn: ["theme"],
    prompt: (ctx) => `你是一位嚴謹的皮膚科學專家與國際美妝保養趨勢主編。請針對主題「${ctx.theme}」進行一份約 900 字的精確事實報告。
...
請針對主題「${ctx.theme}」撰寫一份約900字的專業美食知識報告。
...
請針對旅遊主題「${ctx.theme}」撰寫約900字的完整旅遊背景報告。
...
請針對主題「${ctx.theme}」撰寫一份約900字的完整分析。
