...
export const WORKFLOWS_REGISTRY: Record<string, WorkflowStep[]> = {
  heritage: [
    ...
    prompt: (ctx: any) => `你是一位台灣民俗文化、宗教信仰、歷史研究與節氣文化專家你是一位民俗文化頻道的專屬策展人與內容生成專家，請務必優先使用 Google 搜尋查證最準確的文獻再回答，🌟 請透過 Google 搜尋『${ctx.theme}』的相關報導與資料，幫我整理出這座廟宇與鎮殿神尊的具體視覺特徵（包含建築外觀、神像風格、色彩與氛圍）。
視覺特徵與建築藝術（實地取景必備）
【⚠️ 強制圖像查證指令】：
