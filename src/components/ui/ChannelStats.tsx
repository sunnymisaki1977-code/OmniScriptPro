...
export function ChannelStats() {
  ...
  return (
    <div className="grid grid-cols-3 gap-3 sm:gap-4 mt-8">
      {/* Subscribers */}
      <div className="bg-[#0A2E5C] rounded-2xl p-4 sm:p-6 border border-[#10B981]/30 flex flex-col items-center justify-center text-center shadow-[0_10px_30px_rgba(10,46,92,0.15)] hover:border-[#10B981]/60 transition-colors group">
        <Users className="w-6 h-6 text-[#10B981] mb-2 group-hover:scale-110 transition-transform" />
        <div className="text-xl sm:text-2xl lg:text-3xl font-black text-white">
          {stats ? formatNumber(animatedSubs) : "-"}
        </div>
        <div className="text-xs sm:text-sm text-slate-300 font-medium mt-1">訂閱人數</div>
      </div>
      
      {/* Total Views */}
      <div className="bg-[#0A2E5C] rounded-2xl p-4 sm:p-6 border border-[#10B981]/30 flex flex-col items-center justify-center text-center shadow-[0_10px_30px_rgba(10,46,92,0.15)] hover:border-[#10B981]/60 transition-colors group">
        <Eye className="w-6 h-6 text-[#10B981] mb-2 group-hover:scale-110 transition-transform" />
        <div className="text-xl sm:text-2xl lg:text-3xl font-black text-white">
          {stats ? formatNumber(animatedViews) : "-"}
        </div>
        <div className="text-xs sm:text-sm text-slate-300 font-medium mt-1">總觀看次數</div>
      </div>
      
      {/* Total Videos */}
      <div className="bg-[#0A2E5C] rounded-2xl p-4 sm:p-6 border border-[#10B981]/30 flex flex-col items-center justify-center text-center shadow-[0_10px_30px_rgba(10,46,92,0.15)] hover:border-[#10B981]/60 transition-colors group">
        <Video className="w-6 h-6 text-[#D4AF37] mb-2 group-hover:scale-110 transition-transform" />
        <div className="text-xl sm:text-2xl lg:text-3xl font-black text-white">
          {stats ? animatedVideos : "-"}
        </div>
        <div className="text-xs sm:text-sm text-slate-300 font-medium mt-1">影片總數</div>
      </div>
    </div>
  );
}
